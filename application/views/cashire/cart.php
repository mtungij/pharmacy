       ...
       <?php }else{ ?>
        <div class="pull-right">
          <?php $kwiaha = $this->db->query("SELECT * FROM tbl_store WHERE balance =0"); 
           ?>
      <h2><a href="<?php echo base_url("cashire/get_emptyItm"); ?>">No balance Item<span class="badge badge-danger"><i><?php echo $kwiaha->num_rows(); ?></span></i></a></h2>
      </div>
      <?php } ?>
      </div>
      </div>
...
<table class="table table-hover js-basic-example dataTable table-custom">
            ...
            <b> <?php echo $datas->name; ?> - <?php echo $datas->bland; ?></b>
               ...
               <?php }else{ ?>
                <a href="<?php echo base_url("cashire/addToCart/{$datas->id}"); ?>">
               <b> <?php echo $datas->name; ?> - <?php echo $datas->bland; ?></b>
                <?php } ?> 
...
<table class="table table-hover js-basi-example dataTable table-custom">
    <?php echo form_open("cashire/sell"); ?>
            <thead class="thead-primary">
                <tr>
                    <th>Name</th>
                    <th>Price(Tsh)</th>
     ...
     <!-- <select type="number" class="form-control" id='cartp' onchange="updateCartItem(this, '<?php echo $cart_id; ?>','<?php echo $item_id; ?>')" style="width: 80px">
       ...
       <input type="hidden" name="quantity[]" value="<?php echo $item["qty"]; ?>">
           ...
           <!--  <td><?php //echo $item['subtotal'] - $item['buy_price'] * $item['qty']; ?></td> -->
            <td><a href="<?php echo base_url('cashire_cart/removeItem/'.$item["rowid"]); ?>" class="btn btn-danger btn-sm"><i class=" icon-close"></i></a><?php //echo $products->p_name; ?></td>
              </tr>
    <?php endforeach; ?>
              <tr>
                <th>Total</th>
...
<?php include 'incs/footer.php'; ?>
    ...
    <?php
                ...
                if(isset($_GET['customer'])){
                  ...
                  echo 
                  redirect('cashier/print_recept/'. htmlentities($text));     
                }
                 ?>
<!-- Default Size -->
